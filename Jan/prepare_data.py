...
def prepare_data_t2(num_functions,grid_size,train=True):
    # Parameters
    ...
    def linear_plus_periodic_kernel_matrix(X, lin_variance=1.0, per_length_scale=1.0, per_variance=1.0, p=0.1):
        ...
        return K_lin + K_per 
    def linear_times_periodic_kernel_matrix(X, lin_variance=1.0, per_length_scale=1.0, per_variance=1.0, p=0.1):
        K_lin = linear_kernel_matrix(X, variance=lin_variance)
        K_per = periodic_kernel_matrix(X, length_scale=per_length_scale, variance=per_variance, p=p)
        return K_lin * K_per
    def get_kernel(X,l,v,p):
        ...
        if  r <=0.3:
            ...
            if r>0.6 and r < .8:
                return linear_times_periodic_kernel_matrix(X,v,l,v,p)
            else:
    ...
    for j in tqdm(range(num_functions), desc=f"NUM {i+1}/{100000}", leave=False):
        try:
            ...
            K = get_kernel(X,l,random.uniform(0.5, 2),p)

            mean = np.zeros(grid_size)
            noise_std = np.abs(np.random.normal(0, 0.025))

            samples = np.random.multivariate_normal(mean, K, size=1)#.reshape(-1)
            context_len = int(4/5 * grid_size)
            samples = (samples - np.min(samples[:,:context_len])) / (np.max(samples[:,:context_len]) - np.min(samples[:,:context_len]))
           # print(samples)
            #print(np.max(samples[:context_len]),context_len,samples.shape)
...
def prepare_data_t1(num_functions,grid_size,train=True):
    ...
    for j in tqdm(range(num_functions), desc=f"NUM {i+1}/{100000}", leave=False):
     #   try:
            alpha, beta_param = np.random.choice(dist, 1,replace=False)[0],np.random.choice(dist, 1,replace=False)[0]
            i = j
            l = .1*beta_dist.rvs(alpha, beta_param)
            K = rbf_kernel_matrix(X,l,random.uniform(0.5, 2))
            ...
            train_set_branch_y = np.zeros(shape=(grid_size))
            train_set_branch_t = np.zeros(shape=( grid_size))
            test_truth = np.zeros(shape=( grid_size))

            stats = np.zeros(shape=(9))
            norm_props = np.zeros(shape=(4))


            branch_mask = np.zeros(shape=( grid_size))

            train_set_trunk_t = X.copy().reshape(grid_size)
            ...
            y_observed = samples_noisy[indices]
            y_truth = samples
            t_truth =X.copy()
            t_observed = X.copy()[indices]
        ...
        #    y_mean_truth,y_std_truth = (np.mean(y_truth),np.std(y_truth))     
            ...
            train_set_branch_y = np.append(y_observed_norm,np.zeros(grid_size - len(indices)))         
            train_set_branch_t = np.append(t_observed_norm,np.zeros(grid_size - len(indices)))
            train_set_trunk_t = t_truth
            branch_mask  = np.append(np.ones(len(indices)), np.zeros(grid_size - len(indices)))
            test_truth = y_truth
            stats =np.array([y_min_noise,y_max_noise,
                                y_max_noise - y_min_noise,
                                y_observed[0],y_observed[-1],
                                y_observed[-1] - y_observed[0],
                                t_observed[0],t_observed[-1],
                                t_observed[-1] - t_observed[0]])
            norm_props = np.array([y_min_noise,y_max_noise,np.min(t_observed),np.max(t_observed)])
     ...
     #   except :
      ...
      #      continue
    train_set_branch_y_collection = (np.asarray(train_set_branch_y_collection, dtype=np.float32))
    train_set_branch_t_collection = (np.asarray(train_set_branch_t_collection, dtype=np.float32))
    train_set_trunk_collection = (np.asarray(train_set_trunk_collection, dtype=np.float32))
    branch_mask_collection = (np.asarray(branch_mask_collection, dtype=np.float32))
    test_truth_collection = (np.asarray(test_truth_collection, dtype=np.float32))
    stats_collection = (np.asarray(stats_collection, dtype=np.float32))
    norm_props_collection = (np.asarray(norm_props_collection, dtype=np.float32))


    samples_ = (np.asarray(samples_, dtype=np.float32))
    samples_noisy_ = (np.asarray(samples_noisy_, dtype=np.float32))
...
grid_size = 640
num_f = 30000
data = prepare_data_t2(num_f,grid_size)
